...
def train(config_path):
    ...
    #             T_mult=2,      # Restart interval doubles after each restart (10, 20, 40, ...)
    #             eta_min=1e-6   # Minimum learning rate (prevents zero LR)
    #         )
    # scheduler = lr_scheduler.ReduceLROnPlateau(
    #             optimizer,
    #             mode='min',        # Reduce LR when `loss` stops decreasing
    #             factor=config['train']['gamma'],        # Reduce LR by multiplying it by 
    #             patience=10,       # Wait 10 epochs before reducing LR
    #             threshold=1e-4,    # Minimum change to qualify as an improvement
    #             cooldown=5,        # Number of epochs to wait after reduction before resuming monitoring
    ...
    # scheduler = CyclicLR(optimizer, base_lr=0.000003, max_lr=0.0003, step_size_up=2000, mode='triangular2')
