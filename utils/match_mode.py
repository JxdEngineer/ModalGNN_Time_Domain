...
def match_mode(freq_true, freq_pred, phi_true, phi_pred):
    ...
    MAC_pred_match = []
    for i in range(5):
        if np.min(np.abs(freq_pred - freq_true[i])) > freq_true[i]*0.1: # this mode is not identified, so record empty data
            freq_pred_match.append(0)
            MAC_pred_match.append(0)
        else:
            if np.sum(np.abs(freq_pred - freq_true[i]) < freq_true[i]*0.1) > 1: # more than one possible mode is identified, within the frequency boundary of +-1
                MAC_comparison = []
                match_indexes = np.where(np.abs(freq_pred - freq_true[i]) < freq_true[i]*0.1)[0]
                for j in range(np.sum(np.abs(freq_pred - freq_true[i]) < freq_true[i]*0.1)): # correlate the pair with highest MAC
